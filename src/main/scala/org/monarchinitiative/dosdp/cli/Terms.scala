  ...
  def run(config: TermsConfig): IO[DOSDPError, Unit] =
    Main.withLogContext(Map("command" -> "terms", "pattern" -> config.common.template, "input" -> config.infile, "output" -> config.common.outfile)) {
      for {
        ...
        rows <- ZIO.attempt(CSVReader.open(config.infile, StandardCharsets.UTF_8.name())(sepFormat)).acquireReleaseWithAuto(csvReader => ZIO.attempt(csvReader.iteratorWithHeaders.toList))
