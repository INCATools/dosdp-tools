...
object InternalVarsTest extends ZIOSpecDefault {
  ...
  def spec = suite("Internal vars and multi_value clauses test")(
    test("Utilize internal_vars join function with data_list_vars correctly.") {
      for {
    ...
    test("Utilize internal_vars join function with list_vars correctly.") {
      for {
        dosdp <- Config.inputDOSDPFrom("src/test/resources/org/monarchinitiative/dosdp/internal_vars_test.yaml")
        columnsAndFillers <- Generate.readFillers(new File("src/test/resources/org/monarchinitiative/dosdp/internal_vars_test.tsv"), new TSVFormat {})
        (_, fillers) = columnsAndFillers
        ...
        Harness.assertNoPlaceholderLiterals(axioms) &&
        assert(axioms)(contains(Class("http://purl.obolibrary.org/obo/AllenDendClass_CS202002013_159") Annotation(RDFSLabel, "Sample name with type list: http://purl.obolibrary.org/obo/CL_0011005."))) &&
    ...
    test("Utilize internal_vars and list_vars expanded with the ontology labels correctly.") {
      for {
      ...
      } yield Harness.assertMatchesGolden(axioms, "src/test/resources/org/monarchinitiative/dosdp/internal_vars_test.golden.ofn") &&
        Harness.assertNoPlaceholderIRIs(axioms) &&
        Harness.assertNoPlaceholderLiterals(axioms) &&
        assert(axioms)(contains(Class("http://purl.obolibrary.org/obo/AllenDendClass_CS202002013_159") Annotation(RDFSComment, "Entity label expansion test: MyEntity1_Label."))) &&
    ...
    test("Internal_vars and prefixes expanded with the ontology labels correctly.") {
